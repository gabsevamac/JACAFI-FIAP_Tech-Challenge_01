...
public interface VehicleApi {
    ...
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "400", description = "Invalid plate format or attribute out of range",
                    content = @Content),
    ...
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "401", description = "Missing or invalid JWT", content = @Content),
            @ApiResponse(responseCode = "404", description = "No active vehicle with this identifier",
                    content = @Content)
    })
    VehicleResponse findById(@Parameter(description = "Identifier assigned at registration") UUID id);

    @Operation(summary = "Find by license plate, or list a customer's vehicles",
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Found"),
            @ApiResponse(responseCode = "400", description = "Neither or both query parameters were given",
                    content = @Content),
            @ApiResponse(responseCode = "401", description = "Missing or invalid JWT", content = @Content),
            @ApiResponse(responseCode = "404", description = "No active vehicle with this license plate",
                    content = @Content)
    })
    ...
    @Operation(summary = "Correct make, model and model year",
    ...
    VehicleResponse update(@Parameter(description = "Identifier of the vehicle to correct") UUID id,
                           UpdateVehicleRequest request,
                           Authentication authentication);

    @Operation(summary = "Remove a vehicle from the active registry",
    ...
    ResponseEntity<Void> remove(@Parameter(description = "Identifier of the vehicle to remove") UUID id,
                                Authentication authentication);
}
