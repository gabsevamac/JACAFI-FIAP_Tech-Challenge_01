...
public record RegisterMaterialRequest(
        ...
        @Schema(
                description = "PART for a component that stays in the vehicle, SUPPLY for material "
                        + "consumed during the work",
                example = "PART")
        @NotNull(message = "type is required") MaterialType type,
        ...
        @Schema(
                description = "Opening balance: units already on the shelf. Omit for a material "
                        + "the workshop has yet to buy.",
                example = "12")
        @PositiveOrZero(message = "initialStock must be zero or positive") int initialStock) {}
