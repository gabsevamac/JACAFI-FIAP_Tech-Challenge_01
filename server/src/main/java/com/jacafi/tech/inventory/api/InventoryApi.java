...
public interface InventoryApi {
    ...
    @Operation(
            summary = "Register a part or a supply",
    ...
    @ApiResponses({
        ...
        @ApiResponse(
                responseCode = "409",
                description = "Name already registered to an active material",
                content = @Content)
    })
    ResponseEntity<InventoryItemResponse> register(RegisterMaterialRequest request, Authentication authentication);

    @Operation(summary = "Read one item, with its balance and its open reservations")
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Found"),
        @ApiResponse(responseCode = "401", description = "Missing or invalid JWT", content = @Content),
        @ApiResponse(responseCode = "404", description = "No active item with this identifier", content = @Content)
    })
    ...
    InventoryPageResponse list(
            @Parameter(description = "PART, SUPPLY, or omitted for both") MaterialType type,
            @Parameter(description = "Zero-based page number") int page,
            @Parameter(description = "Page size, at most 100") int size);

    @Operation(
            summary = "Correct the name and the unit price",
    ...
    @ApiResponses({
        ...
        @ApiResponse(
                responseCode = "409",
                description = "Name already registered to another active material",
                content = @Content)
    })
    InventoryItemResponse update(
            @Parameter(description = "Identifier of the item to correct") UUID id,
            UpdateMaterialRequest request,
            Authentication authentication);

    @Operation(
            summary = "Take a material out of the catalogue",
    ...
    @ApiResponses({
        ...
        @ApiResponse(responseCode = "401", description = "Missing or invalid JWT", content = @Content),
        @ApiResponse(responseCode = "404", description = "No active item with this identifier", content = @Content),
        @ApiResponse(responseCode = "409", description = "The item still holds open reservations", content = @Content)
    })
    ResponseEntity<Void> remove(
            @Parameter(description = "Identifier of the item to remove") UUID id, Authentication authentication);

    @Operation(
            summary = "Add units to the shelf",
    ...
    InventoryItemResponse replenish(
            @Parameter(description = "Identifier of the item") UUID id,
            ReplenishStockRequest request,
            Authentication authentication);

    @Operation(
            summary = "Hold units for a service order",
    ...
    @ApiResponses({
        ...
        @ApiResponse(responseCode = "400", description = "Stock out of range or order missing", content = @Content),
        @ApiResponse(responseCode = "401", description = "Missing or invalid JWT", content = @Content),
        @ApiResponse(responseCode = "404", description = "No active item with this identifier", content = @Content),
        @ApiResponse(
                responseCode = "409",
                description = "Fewer units available than were requested",
                content = @Content)
    })
    InventoryItemResponse reserve(
            @Parameter(description = "Identifier of the item") UUID id,
            ReserveMaterialRequest request,
            Authentication authentication);

    @Operation(
            summary = "Give a reservation back",
    ...
    @ApiResponses({
        ...
        @ApiResponse(
                responseCode = "404",
                description = "No active item, or no open reservation " + "for this order",
                content = @Content)
    })
    ...
    @Operation(
            summary = "Take the reserved units off the shelf",
    ...
    StockWithdrawalResponse withdraw(
            @Parameter(description = "Identifier of the item") UUID id,
